...
class AttendanceRepository:
    ...
    async def insert_attendance(self, attendance: Attendance_Member) -> bool:
        try:
            sql = insert(Attendance_Member).values(id=attendance.id, member_id=attendance.member_id, timein=datetime.strptime(
                attendance.timein, "%H:%M"), timeout=datetime.strptime(attendance.timeout, "%H:%M"), date_log=attendance.date_log)
