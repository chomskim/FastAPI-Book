...
def add_tour_destination(input: TourInput):
    try:
        ...
        tour_basic_info = TourBasicInfo(
            id=tid, name=input.name, type=input.type, amenities=input.amenities, ratings=0.0)
...
def list_valuable_visitors():
    try:
        sort_orders = sorted(approved_users.items(),
                             key=lambda x: x[1].booked, reverse=True)
