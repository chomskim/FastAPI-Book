...
async def list_all_cars(
    ...
    full_query = request.app.mongodb[COLLECTION_NAME].find(query).sort(
...
async def update_task(id: str, request: Request, car: CarUpdate = Body(...)):
    ...
    await request.app.mongodb[COLLECTION_NAME].update_one(
        {"_id": id}, {"$set": car.dict(exclude_unset=True)}
    )

    if (car := await request.app.mongodb[COLLECTION_NAME].find_one({"_id": id})) is not None:
        return CarDB(**car)
...
async def brand_price(brand: str, request: Request):
    ...
    full_query = request.app.mongodb[COLLECTION_NAME].aggregate(query)
